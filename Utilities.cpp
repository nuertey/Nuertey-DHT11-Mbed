    ...
    void NetworkStatusCallback(nsapi_event_t status, intptr_t param)
    {
        ...
        switch (param)
        {
            ...
            case NSAPI_STATUS_GLOBAL_UP:
            {
                ...
                //g_pMasterEventQueue->call(&g_NTPClient, &NuerteyNTPClient::SynchronizeRTCTimestamp);
                g_pMasterEventQueue->call_in(100ms, RetrieveNTPTime);            
                
                //g_pMasterEventQueue->call_in(1s, DisplayStatistics);
                
                //g_pMasterEventQueue->call_in(1s, DHT11SensorAcquisition);
                break;
            }
...
// test.mosquitto.org
static const std::string NUERTEY_MQTT_BROKER_ADDRESS("broker.hivemq.com");
